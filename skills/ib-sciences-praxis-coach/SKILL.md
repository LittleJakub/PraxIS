...
- For Mode C, if the teacher has the official subject guide / TSM at hand (e.g. a local copy), offer to consult it for exact descriptor wording; the reference `references/ia-criteria.md` is the built-in digest (identical criteria across the sciences). Corroborate marks against `references/session-subject-reports.md` (examiner feedback across sessions and subjects).
- For uncertainty lookups: `references/equipment-uncertainties.md` has comprehensive specs for Vernier/PASCO sensors, standard glassware, balances, thermometers, and general rules for unknown instruments.
- For statistical decisions: `references/statistics-decision-tree.md` provides a decision tree for which test to use when, per subject, with the n-rules and reporting conventions.
- For grade context: `references/grade-boundaries.md` maps IA component marks to grade boundaries and explains what the marks mean for the student's final grade.
- For mark-capping patterns: `references/anti-patterns.md` is a comprehensive list of what caps marks per criterion, with evidence from examiner reports and the moderation case study.
- For practical feasibility: `references/investigation-feasibility.md` is a topic × equipment × time × level matrix for Mode D and Mode A.
- For calibration context (596 scored reports): `references/calibration-distributions.md` (mark distributions per criterion per subject — what marks actually occur), `references/calibration-band-transitions.md` (features that predict moving between bands), `references/calibration-cross-criterion.md` (criterion correlations — flag inconsistent profiles), `references/calibration-subject-patterns.md` (per-subject means, spreads, grade-7 rates), `references/calibration-investigation-types.md` (scores by hands-on/database/simulation/fieldwork), `references/calibration-topic-intelligence.md` (topic frequency and scoring — which topics are overused), `references/calibration-commentary-intelligence.md` (the deep layer: what the evaluator commentary praises and flags, per subject × criterion × band — read the criterion section before finalizing a mark), and `references/calibration-feedback-library.md` (the fine-grained layer: paraphrased examiner/teacher/student phrasings per specific issue, with frequency and band evidence — the wording source for feedback and mark justifications).
...
5. **Data plan**: a raw-data table skeleton with units and uncertainties in column headings (look up instrument uncertainties in `references/equipment-uncertainties.md`), and a note on data sufficiency for the processing they intend. Use `references/statistics-decision-tree.md` to recommend the appropriate statistical test or processing method for the planned data.
...
2. **Per criterion**: judge the strands against the level descriptors (start at the lowest band), decide the best-fit markband, award a whole number. Mark positively — credit what the student did. A zero only when the report shows nothing at all toward a criterion, or the response is incomprehensible/totally irrelevant; do not over-penalize a single missing strand. Use the subject sample patterns (`references/sample-patterns-biology.md`, `references/sample-patterns-chemistry.md`, `references/sample-patterns-physics.md`) and the subject calibration tables (`references/marking-calibration.md`) to calibrate band decisions (e.g. "no significance test ≈ caps processing strand at 2–3"; a conclusion that contradicts accepted science caps hard). Anchor expectations with the aggregate distributions (`references/calibration-distributions.md` — e.g. only ~9% of biology IAs score 5+ on Data Analysis), the band-transition predictors (`references/calibration-band-transitions.md`), flag inconsistent profiles via `references/calibration-cross-criterion.md`, and read the criterion's band signatures in `references/calibration-commentary-intelligence.md` (what the evaluator commentary actually praises and flags at each band) before finalizing each mark.
...
6. **Add moderation-style comments** (as the TSM recommends): comments on the work give moderators insight into why each mark was given. Use `references/anti-patterns.md` to ensure every mark-capping pattern is explicitly addressed in the justification, and phrase each mark's reason from `references/calibration-feedback-library.md` (its "Band" and "Examiner comment" lines are the ready-made justification language).
7. **Separate deliverables**: the marks + justification (teacher's record) and, if requested, constructive student feedback (Mode B style, actionable). Offer `references/grade-boundaries.md` to contextualise what the IA mark means for the student's final grade.
...
1. **Clarify**: subject, syllabus understanding or topic, level, class time, group size, equipment available, and the lesson's goal (demonstrate a concept, practise a technique, or build toward an IA). Use the inventory file if present, else ask. Consult `references/investigation-feasibility.md` for ready-made options matching the available equipment and time.
...
2. **Run the completeness check per report**: title + candidate code + word count at the start (never student names or school identifiers — examiners flag these); word count ≤ 3,000 and counting only the report text (tables, graphs, headings, references and bibliographies do not count — see `references/data-and-analysis-guide.md`); no appendices beyond consent forms (the full dataset no longer belongs in an appendix — the teacher has reviewed it and records that on the work); the marked criteria table present; teacher comments per criterion on file (the moderator explicitly values them).
